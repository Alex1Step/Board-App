...
const initialState = {
  ...
  const boardsSlice = createSlice({
    ...
    reducers: {
      ...
      moveTask(state, action: PayloadAction<moveTaskI>) {
        let destination = Number(action.payload.destinationBoard)
        let from = Number(action.payload.fromBoard)
        //remember task
        let relocatebleTask = Object.assign( {}, state.boards[from].tasks[action.payload.taskID])
        //change task
  ...
  export const { changeFromInput, boardDeleting, taskDeleting, taskAdd, boardAdd, changeBoardName, moveTask } = boardsSlice.actions
